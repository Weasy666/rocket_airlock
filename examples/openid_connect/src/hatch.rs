...
impl Communicator for CoreClient {
    ...
    async fn from(rocket: Rocket<Build>) -> HatchResult<Self, Self::Error> {
        let config = match rocket
            .figment()
            .extract_inner::<HatchConfig>("airlock.openidconnect")
        {
            Ok(config) => config,
        ...
        info!("Initializing OpenID Client");
...
impl<'h> OidcHatch<'static> {
    pub fn authorize_url(&self) -> (Absolute<'static>, String, String) {
        info!("Generating authorization Url from Manifest with random token and nonce.");
...
impl<'h> Hatch for OidcHatch<'static> {
    ...
    async fn from(rocket: Rocket<Build>) -> HatchResult<OidcHatch<'static>, Self::Error> {
        ...
        let figment = rocket
            ...
            .clone()
            .join((
                "airlock.openidconnect.discover_url",
                config.discover_url.clone(),
            ))
            .join((
                "airlock.openidconnect.redirect_url",
        ...
        Ok((rocket.reconfigure(figment), oidc_hatch))
    }
}
...
pub fn login(airlock: Airlock<OidcHatch<'static>>, cookies: &CookieJar<'_>) -> Redirect {
    ...
    cookies.add_private(Cookie::build(("oicd_state", csrf_state)).same_site(SameSite::Lax));
    cookies.add_private(Cookie::build(("oicd_nonce", nonce)).same_site(SameSite::Lax));

    info!("Redirecting to {}", Paint::new(&authorize_url).underline());
...
pub(crate) async fn login_callback(
    ...
    debug!("[login_callback] Returned code: {}", &auth_response.code);
    ...
    cookies.add_private(
        Cookie::build(("oicd_access_token", claim_resonse.access_token)).same_site(SameSite::Lax),
    );
...
impl<'r> FromRequest<'r> for AuthenticationResponse {
    ...
    async fn from_request(request: &'r Request<'_>) -> Outcome<Self, Self::Error> {
        ...
        let session_state: Option<String> = request
        ...
        let auth_response = match (code, state, session_state) {
            (Some(code), Some(state), Some(session_state)) => {
                ...
                match state_cookie {
                    ...
                    Some(_) => warn!("The stored state differs from the state returned from the OpenID Provider."),
                ...
                let nonce = match nonce_cookie {
                    ...
                    _ => {
                        warn!("No nonce was stored for the current auth flow.");
            ...
            _ => {
                info!("Missing on providers respones: {}", missing.join(", "));
