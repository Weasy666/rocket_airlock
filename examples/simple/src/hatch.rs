...
impl SimpleHatch {
    pub fn authenticate_username(&self, username: &str) -> bool {
        ...
        info!(
            "Authenticating '{}' against valid user '{}'",
...
pub fn login(
    airlock: Airlock<SimpleHatch>,
    username: String,
    cookies: &CookieJar<'_>,
) -> Result<Redirect, Status> {
    info!("Someone tries to log in with username: {}", &username);
    match airlock.hatch.authenticate_username(&username) {
        true => {
            info!("Authentication successfull!");
            cookies.add_private(Cookie::build(("logged_in", username)).same_site(SameSite::Lax));
            Ok(Redirect::to("/"))
        }
